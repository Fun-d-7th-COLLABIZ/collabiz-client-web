...
function LeftSidebarGuide() {
  ...
  return (
    <div className="mg-y-30 py-5 px-2"
      style={{borderRadius: `${20}px`, backgroundColor: "#F7F7F7", width: `${198}px`, height: `${685}px`}}
    >
      ...
      >
        {[
          ...
          {link: '/docs/privacy-policy'  , name: '개인정보처리및방침'},
          {link: '/qna'                  , name: '문의하기'       },
        ].map((v, i) => {
          return (
            <NavLink key={i}
              to={{pathname: v.link}}
              state={{scrollTo: [0, 0]}}
              className={"nav-link" + (v.link === location.pathname ? " fw-600"  : "")}
            >
              · {v.name}
            </NavLink>
          );
